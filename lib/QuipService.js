...
class QuipService {
    ...
    async checkUser() {
        ...
        if (!res.ok) {
            ...
            console.log(colors.red(`Error ${res.status}: ${res.statusText}`))
            const retryAfter = +res.headers.get('retry-after');
            if (retryAfter > 0) {
                console.log(colors.red(`Retry after: ${retryAfter} seconds (${retryAfter/60} minutes)`));
                this.logger.error(`Retry after: ${retryAfter} seconds (${retryAfter/60} minutes)`);
            }
            return false;
        }
        ...
        try {
            ...
            if(!res.ok) {
                if(res.status === 503 || res.status === 429) {
                    ...
                    const retryAfterMs = +res.headers.get('retry-after')*1000;
                    const rateLimitReset = +res.headers.get('x-ratelimit-reset')*1000;
                    
                    let waitingInMs = this.waitingMs;
                    if (retryAfterMs > 0) {
                        waitingInMs = retryAfterMs;
                    }
                    else if(rateLimitReset > currentTime) {
                        waitingInMs = rateLimitReset - currentTime;
                    }
                    this.logger.info(`HTTP 429: for ${url}, waiting in ms: ${waitingInMs} based on headers`);
                    this.logger.info(`Retry after: ${waitingInMs} ms / ${waitingInMs/1000} seconds (${waitingInMs/60000} minutes)`);
